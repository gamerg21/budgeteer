from . import db

class Month(db.Model):
    __tablename__ = 'months'

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), nullable=False)
    year = db.Column(db.Integer, nullable=False)
    start_date = db.Column(db.Date, nullable=False)
    end_date = db.Column(db.Date, nullable=False)

    expenses = db.relationship('Expense', back_populates='month', cascade='all, delete-orphan')
    statements = db.relationship('Statement', back_populates='month', cascade='all, delete-orphan')

    def __repr__(self):
        return f'<Month {self.name} {self.year}>'


class CreditCard(db.Model):
    __tablename__ = 'credit_cards'

    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    issuer = db.Column(db.String(100))
    last4 = db.Column(db.String(4))
    credit_limit = db.Column(db.Float)

    expenses = db.relationship('Expense', back_populates='card', cascade='all, delete-orphan')
    statements = db.relationship('Statement', back_populates='card', cascade='all, delete-orphan')

    def __repr__(self):
        return f'<CreditCard {self.name}>'


class Expense(db.Model):
    __tablename__ = 'expenses'

    id = db.Column(db.Integer, primary_key=True)
    month_id = db.Column(db.Integer, db.ForeignKey('months.id'), nullable=False)
    card_id = db.Column(db.Integer, db.ForeignKey('credit_cards.id'), nullable=False)
    date = db.Column(db.Date, nullable=False)
    description = db.Column(db.String(255), nullable=False)
    amount = db.Column(db.Float, nullable=False)
    category = db.Column(db.String(100))

    month = db.relationship('Month', back_populates='expenses')
    card = db.relationship('CreditCard', back_populates='expenses')

    def __repr__(self):
        return f'<Expense {self.description} {self.amount}>'


class Statement(db.Model):
    __tablename__ = 'statements'

    id = db.Column(db.Integer, primary_key=True)
    card_id = db.Column(db.Integer, db.ForeignKey('credit_cards.id'), nullable=False)
    month_id = db.Column(db.Integer, db.ForeignKey('months.id'), nullable=False)
    statement_balance = db.Column(db.Float, nullable=False)
    due_date = db.Column(db.Date, nullable=False)
    payment_made = db.Column(db.Float, default=0.0)

    card = db.relationship('CreditCard', back_populates='statements')
    month = db.relationship('Month', back_populates='statements')

    def __repr__(self):
        return f'<Statement card={self.card_id} month={self.month_id}>'
